...
1. Reuse one ``PolicyNet`` and one compatibility ``qnet`` module instead of
   rebuilding TensorFlow objects for every candidate parameter evaluation.
...
class _TensorModelParams:
    ...
    def __init__(self, mp: ModelParams) -> None:
        ...
        self.r_c = tf.constant(getattr(mp, "r_c", mp.r), dtype=tf.float32)
...
class ReusableInnerObjective1Solver:
    ...
    The solver owns one policy network, one compatibility pricing module, and
    one pair of optimizers. For each candidate parameter vector, it restores the initial
    ...
    def __init__(
        ...
        ``initial_policy_weights`` and ``initial_q_weights`` implement the
        compatible warm-start device used during estimation: every candidate
        parameter vector is solved from the same already-trained baseline
        network weights when they are available.  This reduces inner-solve cost
        without changing the structural estimator, because each candidate is
        still re-optimized with fixed random numbers under its own parameters.
        """
        self.template_mp = mp
        self.npol = npol
        self.nq = nq
        ...
        if initial_policy_weights is not None:
            self.policy.set_weights([np.asarray(w).copy() for w in initial_policy_weights])
        if initial_q_weights is not None:
            self.qnet.set_weights([np.asarray(w).copy() for w in initial_q_weights])
        self.initialization_source = (
            "baseline_trained_warm_start" if initial_policy_weights is not None else "deterministic_random_initialization"
        )
    ...
    def computational_controls(self) -> Dict[str, object]:
        """Return metadata describing estimation-time feasibility controls."""
        return {
            "inner_solver": "ReusableInnerObjective1Solver",
            "initialization_source": self.initialization_source,
            "fixed_common_random_numbers": True,
            "candidate_cache": True,
            "inner_epochs": int(self.tp_inner.epochs),
            "inner_steps_per_epoch": int(self.tp_inner.steps_per_epoch),
            "inner_train_paths": int(self.tp_inner.N_paths_train),
            "inner_train_horizon": int(self.tp_inner.T_train),
            "inner_pricing_mc_draws": int(self.tp_inner.N_q),
        }
    ...
    def _fixed_draw_obj1_loss(self) -> Tuple[tf.Tensor, tf.Tensor, tf.Tensor]:
        ...
        def body(
            ...
            q = _price_q(self.qnet, z_cur, k_next, b_next, self.model_state, self.tp_inner)
        ...
        q0 = _price_q(self.qnet, z0, k1, b1, self.model_state, self.tp_inner)
...
def finite_horizon_predefault_continuation(
    ...
    for h in range(horizon):
        ...
        q_raw = _price_q(qnet, z_cur, k_next_raw, b_next_raw, mp, tp)
        eps_h = eps_future[:, h]
