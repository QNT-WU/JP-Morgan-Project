...
def jacobian_singular_value_report(
    jacobian: np.ndarray,
    *,
    param_names: Sequence[str],
    ...
    """Summarize local-identification information from a moment Jacobian.

    The written risky-debt summary requires Jacobian-rank and singular-value
    diagnostics for both the GMM and SMM moment maps.  This helper keeps that
    reporting centralized and deterministic: it computes the singular values of
    the supplied moment-by-parameter Jacobian, a numerical rank, a full-column
    rank indicator, and a condition number.

    Parameters
    ----------
    jacobian:
        Matrix whose rows are moment conditions and whose columns are estimated
        structural parameters.
    param_names:
        Ordered names of the estimated parameters, usually
        ``(theta, psi0, alpha)``.
    moment_names:
        Optional ordered moment names.  They are stored for traceability but are
        not required for the singular-value computation.
    rank_tol:
        Optional rank tolerance.  If omitted, a NumPy/LAPACK-style tolerance
        based on matrix shape and the largest singular value is used.

    Returns
    -------
    dict
        JSON-serializable diagnostics suitable for report tables and plots.
    """
    j = np.asarray(jacobian, dtype=float)
    if j.ndim != 2:
        j = np.atleast_2d(j)
    n_moments, n_params = int(j.shape[0]), int(j.shape[1])
    if j.size == 0 or n_params == 0:
    ...
    if rank_tol is None:
        ...
        rank_tol = float(max(j.shape) * eps * max(max_sv, 1.0))
    rank = int(np.sum(singular_values > float(rank_tol)))
    min_sv = float(singular_values[-1]) if singular_values.size else float("nan")
    if singular_values.size and np.isfinite(min_sv) and min_sv > 0.0:
        cond = float(max_sv / min_sv)
    else:
        cond = float("inf")
    return {
        "param_names": list(param_names),
        "moment_names": list(moment_names) if moment_names is not None else [],
        "shape": [n_moments, n_params],
        "singular_values": [float(x) for x in singular_values],
        "rank_tol": float(rank_tol),
        "numerical_rank": rank,
        "n_params": n_params,
        "n_moments": n_moments,
        "full_column_rank": bool(rank >= n_params),
