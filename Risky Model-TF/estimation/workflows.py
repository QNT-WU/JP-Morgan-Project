...
class FrequentistEstimationWorkflow:
    ...
    def _load_smm_results_if_complete(self) -> Optional[Dict[str, object]]:
        """Load a previously finalized SMM result, if available.

        This avoids rerunning completed SMM stages when the user only needs to
        resume or start GMM.  The completed optimizer-start cache remains useful
        for recovery, but a finalized ``smm_results.json`` is the cleanest
        checkpoint for method-level resume.
        """
        path = os.path.join(self.out_dir, "smm_results.json")
        if not os.path.exists(path):
            return None
        with open(path, "r", encoding="utf-8") as fh:
            payload = json.load(fh)
        print(f"[SMM] loaded finalized SMM results from {path}; skipping SMM rerun.", flush=True)
        return payload

    def _load_shared_synthetic_data(self) -> Dict[str, np.ndarray]:
        """Load the shared synthetic dataset required by GMM."""
        synth_path = os.path.join(self.out_dir, "smm_synth_data.npz")
        if not os.path.exists(synth_path):
            raise FileNotFoundError(
                f"GMM requires the shared synthetic dataset at {synth_path}. "
                "Run SMM once, or copy the completed estimation output folder before using --estimation_method gmm."
            )
        return dict(np.load(synth_path))

    def run(self, *, policy_true, qnet_true, benchmark=None) -> Dict[str, object]:
        """Run the requested frequentist estimator(s) and save summaries."""
        os.makedirs(self.out_dir, exist_ok=True)
        # Compatible feasibility device: initialize every candidate inner solve
        # from the already-trained baseline networks when available.  This is a
        # deterministic warm start, not a surrogate estimator: each candidate
        # parameter vector is still re-optimized under fixed random numbers.
        initial_policy_weights = policy_true.get_weights() if hasattr(policy_true, "get_weights") else None
        initial_q_weights = qnet_true.get_weights() if hasattr(qnet_true, "get_weights") else None
        method = str(self.config.estimation_method).lower()
        if method not in {"both", "smm", "gmm"}:
            raise ValueError(f"Unknown estimation_method={self.config.estimation_method!r}.")

        smm_res: Dict[str, object] = {}
        if method in {"both", "gmm"}:
            smm_res = self._load_smm_results_if_complete() or {}

        if method in {"both", "smm"} and not smm_res:
        ...
        gmm_res: Dict[str, object] = {}
        if method in {"both", "gmm"}:
            data = self._load_shared_synthetic_data()
            ...
            gmm_res = estimate_gmm(
                ...
                n_starts=self.config.n_starts,
                continuation_horizon=self.config.continuation_horizon,
                moment_mode=self.config.gmm_moment_mode,
                report_mode=self.config.estimation_report_mode,
        ...
        comparison = {
            ...
            "gmm_moment_mode": str(self.config.gmm_moment_mode),
            "estimation_method": method,
            "estimation_report_mode": str(self.config.estimation_report_mode),
            ...
            "computational_controls": {
                "baseline_warm_start": bool(initial_policy_weights is not None),
                "fixed_common_random_numbers": True,
                "candidate_caching": True,
                "inner_epochs": int(self.config.inner_epochs),
                "inner_steps": int(self.config.inner_steps),
                "sim_T": int(self.config.sim_T),
                "sim_burn": int(self.config.sim_burn),
                "sim_n_paths": int(self.config.sim_n_paths),
            },
            "gmm_variants": {
                k: {
                    "objective": v.get("objective"),
