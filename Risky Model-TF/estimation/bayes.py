...
class BayesianRiskyDebtEstimator:
    ...
    def _prior_summary(self) -> Dict[str, Dict[str, float | str]]:
        """Return analytic prior summaries for report-ready comparison.

        The prior-to-posterior table uses these values to show whether the
        synthetic data move each posterior away from its prior location and
        reduce prior uncertainty.
        """
        psi0_loc = float(np.log(float(self.mp_base.psi0)))
        psi0_scale = 0.75
        summaries: Dict[str, Dict[str, float | str]] = {
            "theta": {
                "family": "Beta(2,2)",
                "mean": float(self.priors["theta"].mean().numpy()),
                "std": float(self.priors["theta"].stddev().numpy()),
            },
            "psi0": {
                "family": "LogNormal(log(psi0_true),0.75)",
                "mean": float(self.priors["psi0"].mean().numpy()),
                "std": float(self.priors["psi0"].stddev().numpy()),
                "loc": psi0_loc,
                "scale": psi0_scale,
            },
            "alpha": {
                "family": "Beta(2,2)",
                "mean": float(self.priors["alpha"].mean().numpy()),
                "std": float(self.priors["alpha"].stddev().numpy()),
            },
        }
        return summaries
    ...
    def run(self, out_dir: str, figures_dir: str | None = None) -> Dict[str, object]:
        ...
        def _make_kernel(num_burnin_steps: int):
            """Build a TFP kernel for one resumable MCMC segment."""
            if self.mcmc_cfg.kernel == "hmc":
                ...
                if int(num_burnin_steps) > 0:
                    return tfp.mcmc.SimpleStepSizeAdaptation(
                        inner_kernel=base_kernel,
                        num_adaptation_steps=max(1, int(0.8 * int(num_burnin_steps))),
        ...
        def _trace_fn(_, pkr):
            """Trace acceptance and target density for HMC or RWM kernels."""
            if self.mcmc_cfg.kernel == "hmc" and hasattr(pkr, "inner_results"):
                return {
        ...
        resume_dir = os.path.join(out_dir, "bayes_resume")
        os.makedirs(resume_dir, exist_ok=True)
        resume_path = os.path.join(resume_dir, "chain_latest.npz")
        meta_path = os.path.join(resume_dir, "LATEST.json")
        draws_chunks = []
        accepted_chunks = []
        logprob_chunks = []
        completed = 0
        current_state_np = init_u
        if os.path.exists(resume_path):
            try:
                prev = np.load(resume_path, allow_pickle=False)
                prev_draws = np.asarray(prev["draws_u"], dtype=np.float32)
                prev_acc = np.asarray(prev["accepted"], dtype=bool)
                prev_lp = np.asarray(prev["target_log_prob"], dtype=np.float32)
                if prev_draws.ndim == 3 and prev_draws.shape[1:] == init_u.shape:
                    completed = min(int(prev_draws.shape[0]), int(self.mcmc_cfg.num_results))
                    if completed > 0:
                        draws_chunks.append(prev_draws[:completed])
                        accepted_chunks.append(prev_acc[:completed])
                        logprob_chunks.append(prev_lp[:completed])
                        current_state_np = prev_draws[completed - 1]
                        print(f"[BayesResume] resumed {completed}/{self.mcmc_cfg.num_results} saved draws.", flush=True)
            except Exception:
                completed = 0
                current_state_np = init_u
                draws_chunks = []
        ...
        segment_size = max(1, min(5, int(self.mcmc_cfg.num_results)))
        segment_index = 0
        while completed < int(self.mcmc_cfg.num_results):
            n_segment = min(segment_size, int(self.mcmc_cfg.num_results) - completed)
            burnin = int(self.mcmc_cfg.num_burnin) if completed == 0 else 0
            kernel = _make_kernel(burnin)
            draws_seg, trace_seg = tfp.mcmc.sample_chain(
                num_results=n_segment,
                num_burnin_steps=burnin,
                current_state=tf.convert_to_tensor(current_state_np, dtype=tf.float32),
                kernel=kernel,
                trace_fn=_trace_fn,
                seed=int(self.mcmc_cfg.seed + 1000 + segment_index),
            )
            draws_seg_np = draws_seg.numpy()
            acc_seg_np = np.asarray(trace_seg["is_accepted"], dtype=bool)
            lp_seg_np = np.asarray(trace_seg["target_log_prob"], dtype=np.float32)
            draws_chunks.append(draws_seg_np)
            accepted_chunks.append(acc_seg_np)
            logprob_chunks.append(lp_seg_np)
            completed += int(draws_seg_np.shape[0])
            current_state_np = draws_seg_np[-1]
            draws_u_so_far = np.concatenate(draws_chunks, axis=0)
            accepted_so_far = np.concatenate(accepted_chunks, axis=0)
            logprob_so_far = np.concatenate(logprob_chunks, axis=0)
            np.savez_compressed(
                resume_path,
                draws_u=draws_u_so_far,
                accepted=accepted_so_far,
                target_log_prob=logprob_so_far,
                current_state=current_state_np,
            )
            with open(meta_path, "w", encoding="utf-8") as fh:
                json.dump(
                    {
                        "component": "bayesian_mcmc",
                        "kernel": self.mcmc_cfg.kernel,
                        "completed_draws": int(completed),
                        "requested_draws": int(self.mcmc_cfg.num_results),
                        "segmented_resume": True,
                        "burnin_applied_in_first_segment": int(self.mcmc_cfg.num_burnin),
                    },
                    fh,
                    indent=2,
                )
            print(f"[BayesResume] saved {completed}/{self.mcmc_cfg.num_results} draws.", flush=True)
            segment_index += 1

        draws_u_np = np.concatenate(draws_chunks, axis=0)[: int(self.mcmc_cfg.num_results)]
        ...
        trace = {
            "is_accepted": tf.convert_to_tensor(np.concatenate(accepted_chunks, axis=0)[: int(self.mcmc_cfg.num_results)]),
            "target_log_prob": tf.convert_to_tensor(np.concatenate(logprob_chunks, axis=0)[: int(self.mcmc_cfg.num_results)], dtype=tf.float32),
        }
        ...
        credible_interval_coverage = self._credible_interval_coverage(
        ...
        results = {
            ...
            "posterior_summary": posterior_summary,
            "prior_summary": self._prior_summary(),
            ...
            "checkpoint_resume": {
                "enabled": True,
                "mode": "segmented_hmc_resume_from_last_theta" if self.mcmc_cfg.kernel == "hmc" else "segmented_rwm_resume_from_last_theta",
                "resume_dir": resume_dir,
                "completed_draws": int(draws_u_np.shape[0]),
            },
        }

        accepted_np = np.concatenate(accepted_chunks, axis=0)[: int(self.mcmc_cfg.num_results)]
        target_log_prob_np = np.concatenate(logprob_chunks, axis=0)[: int(self.mcmc_cfg.num_results)]
    ...
    @staticmethod
    def _credible_interval_coverage(*, posterior_summary: Dict[str, Dict[str, float]], true_parameters: Dict[str, float]) -> Dict[str, Dict[str, object]]:
        """Return 90 percent credible-interval coverage flags for true values."""
        coverage: Dict[str, Dict[str, object]] = {}
        for name, stats in posterior_summary.items():
            true_value = float(true_parameters[name])
            lower = float(stats["p05"])
            upper = float(stats["p95"])
            ...
            coverage[name] = {
                ...
                "ci_lower": lower,
                "ci_upper": upper,
                "covered_90pct": bool(lower <= true_value <= upper),
                "posterior_mean": mean,
                "posterior_median": float(stats["median"]),
                "absolute_error_mean": abs_error,
                "relative_error_mean": None if abs(true_value) < 1e-12 else abs_error / abs(true_value),
            }
        return coverage
