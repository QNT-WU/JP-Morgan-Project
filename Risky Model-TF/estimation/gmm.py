...
class _GMMEvaluator:
    ...
    def __init__(
        ...
        self.inner = ReusableInnerObjective1Solver(mp_true, npol, nq, tp_inner, seed=seed + 100)
        self.seed = int(seed)
    ...
    def evaluate(self, x: Sequence[float]) -> Dict[str, object]:
        ...
        out = {"mp": mp_cand, "network_snapshot": snapshot, "m_series": m_series, "g": g}
        self.cache[key] = out
        return out
    ...
    def _moment_contributions(self, policy, qnet, mp: ModelParams) -> np.ndarray:
        ...
        with tf.GradientTape(persistent=True) as tape_choice:
            ...
            q_t = qnet(tf.stack([ds.z_t, ds.k1, ds.b1], axis=1))
...
def _finalize_gmm_variant(
    ...
    mean_zp_mom = np.mean(zp_block, axis=0)
    ...
    q_obs = np.asarray(qnet_hat(tf.stack([ds.z_t, ds.k1, ds.b1], axis=1)).numpy(), dtype=float)
...
def _build_gmm_identification_report(
    ...
    for i, name in enumerate(param_names):
        ...
        for val in grid:
            ...
            mean_abs_zp.append(float(np.mean(np.abs(zp_block))))
            mean_u_k.append(float(np.mean(np.abs(u_k_block))))
            mean_u_b.append(float(np.mean(np.abs(u_b_block))))
        sweeps[name] = {
            "grid": grid.tolist(),
...
def estimate_gmm(
    ...
    """Run plan-aligned two-step GMM with A/B covariance variants."""
