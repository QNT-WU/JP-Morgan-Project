...
def _safe_float(x, default: float = float("nan")) -> float:
    """Convert a value to ``float`` while preserving a fallback default."""
    try:
        if x is None:
            return default
        return float(x)
...
def _safe_bool(x) -> bool:
    """Convert a value to ``bool`` without raising on malformed inputs."""
    try:
        return bool(x)
    except Exception:
        return False
...
def _format_value(value: object, decimals: int = 6) -> str:
    """Format scalars for compact CSV and TeX reporting."""
    if value is None:
        return ""
...
def _write_tex_table(path: str, rows: Sequence[Mapping[str, object]], fieldnames: Sequence[str], caption: Optional[str] = None, label: Optional[str] = None) -> None:
    """Write a simple LaTeX tabular/table artifact from report rows."""
    os.makedirs(os.path.dirname(path), exist_ok=True)
...
class RiskyDebtEstimationReportWriter:
    """Create report-ready artifacts from risky-debt estimation results."""

    def __init__(self, *, out_dir: str, report_mode: str = "full") -> None:
        self.out_dir = out_dir
        self.report_mode = str(report_mode).lower().strip()
        if self.report_mode not in {"full", "light"}:
    ...
    def write_all(self, *, gmm_res: Optional[Mapping[str, object]] = None, smm_res: Optional[Mapping[str, object]] = None, bayes_res: Optional[Mapping[str, object]] = None) -> Dict[str, object]:
        """Write all available frequentist and Bayesian report artifacts."""
        manifest: Dict[str, object] = {"report_mode": self.report_mode}
    ...
    def _write_frequentist_reports(self, *, gmm_res: Mapping[str, object], smm_res: Mapping[str, object]) -> Dict[str, object]:
        ...
        if self.report_mode == "light":
            print("[EstimationReport] light mode: wrote core parameter/convergence/comparison tables; skipping heavy pricing, Jacobian, and plot diagnostics.", flush=True)
        ...
        self._write_table_pair(stem="table_statistical_tests", rows=self._statistical_test_rows(method_records), fieldnames=["method", "family", "statistic_name", "statistic", "dof", "p_value", "reject_at_5_percent", "final_success"], caption="Risky-debt overidentification and specification tests.", label="tab:risky_statistical_tests")
        self._write_table_pair(stem="table_jacobian_rank_diagnostics", rows=self._jacobian_rank_rows(method_records), fieldnames=["method", "family", "n_moments", "n_params", "numerical_rank", "full_column_rank", "min_singular_value", "max_singular_value", "condition_number", "rank_tol", "final_success"], caption="Risky-debt GMM/SMM moment-Jacobian rank diagnostics.", label="tab:risky_jacobian_rank")
        self._write_table_pair(stem="table_jacobian_singular_values", rows=self._jacobian_singular_value_rows(method_records), fieldnames=["method", "family", "singular_index", "singular_value", "log10_singular_value", "rank_tol", "final_success"], caption="Risky-debt GMM/SMM moment-Jacobian singular values.", label="tab:risky_jacobian_singular_values")
        _write_csv(os.path.join(self.out_dir, "raw_results.csv"), raw_results_rows, list(raw_results_rows[0].keys()) if raw_results_rows else ["method"])
        _write_csv(os.path.join(self.out_dir, "raw_start_results.csv"), raw_start_rows, list(raw_start_rows[0].keys()) if raw_start_rows else ["method"])
        ...
        self._plot_jacobian_singular_values(method_records, os.path.join(self.out_dir, "plot_jacobian_singular_values.png"))
        return {
            "estimation_results_json": estimation_results_path,
            "estimation_summary_json": estimation_summary_path,
            "raw_results_csv": os.path.join(self.out_dir, "raw_results.csv"),
            ...
            "table_statistical_tests_csv": os.path.join(self.out_dir, "table_statistical_tests.csv"),
            "table_statistical_tests_tex": os.path.join(self.out_dir, "table_statistical_tests.tex"),
            "table_jacobian_rank_diagnostics_csv": os.path.join(self.out_dir, "table_jacobian_rank_diagnostics.csv"),
            "table_jacobian_rank_diagnostics_tex": os.path.join(self.out_dir, "table_jacobian_rank_diagnostics.tex"),
            "table_jacobian_singular_values_csv": os.path.join(self.out_dir, "table_jacobian_singular_values.csv"),
            "table_jacobian_singular_values_tex": os.path.join(self.out_dir, "table_jacobian_singular_values.tex"),
            ...
            "plot_jacobian_singular_values_png": os.path.join(self.out_dir, "plot_jacobian_singular_values.png"),
        }
    ...
    def _extract_method_record(self, *, method: str, family: str, weight_method: str, method_payload: Mapping[str, object], stage1_payload: Mapping[str, object]) -> Dict[str, object]:
        ...
        return {
            ...
            "jacobian_diagnostics": dict(method_payload.get("jacobian_diagnostics", {})),
            ...
            "specification_dof": int(_safe_float(method_payload.get("specification_dof"), 0.0)),
            "specification_p_value": _safe_float(method_payload.get("specification_p_value"), np.nan),
            "specification_reject_5pct": _safe_bool(method_payload.get("specification_reject_5pct", False)),
            "J_stat": _safe_float(method_payload.get("J_stat"), np.nan),
            "J_dof": int(_safe_float(method_payload.get("J_dof"), 0.0)),
            "J_p_value": _safe_float(method_payload.get("J_p_value"), np.nan),
            "J_reject_5pct": _safe_bool(method_payload.get("J_reject_5pct", False)),
        }
    ...
    def _build_estimation_results_json(self, method_records: Sequence[Mapping[str, object]]) -> Dict[str, object]:
        ...
        for rec in method_records:
            ...
            out[rec["method"]] = {
                ...
                "jacobian_diagnostics": rec.get("jacobian_diagnostics", {}),
    ...
    def _statistical_test_rows(self, method_records: Sequence[Mapping[str, object]]) -> List[Dict[str, object]]:
        """Return overidentification/specification test rows with p-values."""
        rows: List[Dict[str, object]] = []
        for rec in method_records:
            if rec.get("family") == "GMM":
                rows.append({
                    "method": rec["method"],
                    "family": rec["family"],
                    "statistic_name": "GMM J",
                    "statistic": rec.get("J_stat"),
                    "dof": rec.get("J_dof"),
                    "p_value": rec.get("J_p_value"),
                    "reject_at_5_percent": rec.get("J_reject_5pct"),
            ...
            if rec.get("family") == "SMM":
                rows.append({
                    "method": rec["method"],
                    "family": rec["family"],
                    "statistic_name": "SMM specification",
                    "statistic": rec.get("specification_stat"),
                    "dof": rec.get("specification_dof"),
                    "p_value": rec.get("specification_p_value"),
                    "reject_at_5_percent": rec.get("specification_reject_5pct"),
                    "final_success": rec.get("final_success"),
                })
        return rows

    def _jacobian_rank_rows(self, method_records: Sequence[Mapping[str, object]]) -> List[Dict[str, object]]:
        """Return one local-identification rank row per GMM/SMM method.

        The written summary asks for Jacobian-rank diagnostics to verify local
        identification of the three estimated risky-debt parameters.  This table
        records whether the numerical moment Jacobian has full column rank and
        how ill-conditioned that Jacobian is.
        """
        rows: List[Dict[str, object]] = []
        for rec in method_records:
            diag = dict(rec.get("jacobian_diagnostics", {}))
            rows.append({
                "method": rec.get("method"),
                "family": rec.get("family"),
                "n_moments": diag.get("n_moments"),
                "n_params": diag.get("n_params"),
                "numerical_rank": diag.get("numerical_rank"),
                "full_column_rank": diag.get("full_column_rank"),
                "min_singular_value": diag.get("min_singular_value"),
                "max_singular_value": diag.get("max_singular_value"),
                "condition_number": diag.get("condition_number"),
                "rank_tol": diag.get("rank_tol"),
                "final_success": rec.get("final_success"),
            })
        return rows

    def _jacobian_singular_value_rows(self, method_records: Sequence[Mapping[str, object]]) -> List[Dict[str, object]]:
        """Return long-format singular-value rows for moment Jacobians."""
        rows: List[Dict[str, object]] = []
        for rec in method_records:
            diag = dict(rec.get("jacobian_diagnostics", {}))
            rank_tol = _safe_float(diag.get("rank_tol"), np.nan)
            for i, sv in enumerate(diag.get("singular_values", []) or [], start=1):
                sv_float = _safe_float(sv, np.nan)
                rows.append({
                    "method": rec.get("method"),
                    "family": rec.get("family"),
                    "singular_index": i,
                    "singular_value": sv_float,
                    "log10_singular_value": float(np.log10(max(sv_float, 1.0e-300))) if np.isfinite(sv_float) else np.nan,
                    "rank_tol": rank_tol,
                    "final_success": rec.get("final_success"),
                })
        return rows
    ...
    def _write_bayesian_reports(self, *, bayes_res: Mapping[str, object]) -> Dict[str, object]:
        ...
        prior_posterior_rows = self._bayes_prior_to_posterior_rows(bayes_res)
        coverage_rows = self._bayes_coverage_rows(bayes_res)
        self._write_table_pair(stem="table_bayes_posterior_summary", rows=posterior_rows, fieldnames=["parameter", "true_value", "posterior_mean", "posterior_median", "posterior_std", "p05", "p95", "covered_90pct", "abs_error", "relative_error"], caption="Risky-debt Bayesian posterior summary with credible-interval coverage.", label="tab:risky_bayes_posterior_summary")
        self._write_table_pair(stem="table_bayes_prior_to_posterior", rows=prior_posterior_rows, fieldnames=["parameter", "prior_family", "prior_mean", "prior_std", "posterior_mean", "posterior_std", "posterior_median", "posterior_p05", "posterior_p95", "posterior_to_prior_sd_ratio", "posterior_minus_prior_mean"], caption="Risky-debt Bayesian prior-to-posterior updating diagnostics.", label="tab:risky_bayes_prior_to_posterior")
        self._write_table_pair(stem="table_bayes_credible_interval_coverage", rows=coverage_rows, fieldnames=["parameter", "true_value", "ci_level", "ci_lower", "ci_upper", "covered_90pct", "posterior_mean", "posterior_median", "absolute_error_mean", "relative_error_mean"], caption="Risky-debt Bayesian credible-interval coverage diagnostics.", label="tab:risky_bayes_coverage")
        ...
        with open(bayes_summary_path, "w", encoding="utf-8") as f:
            json.dump(make_json_serializable({"posterior_rows": posterior_rows, "prior_to_posterior_rows": prior_posterior_rows, "coverage_rows": coverage_rows, "diagnostic_rows": diagnostic_rows, "kernel": bayes_res.get("kernel"), "sampler": bayes_res.get("sampler"), "filter": bayes_res.get("filter")}), f, indent=2)
        return {"table_bayes_posterior_summary_csv": os.path.join(self.out_dir, "table_bayes_posterior_summary.csv"), "table_bayes_posterior_summary_tex": os.path.join(self.out_dir, "table_bayes_posterior_summary.tex"), "table_bayes_prior_to_posterior_csv": os.path.join(self.out_dir, "table_bayes_prior_to_posterior.csv"), "table_bayes_prior_to_posterior_tex": os.path.join(self.out_dir, "table_bayes_prior_to_posterior.tex"), "table_bayes_credible_interval_coverage_csv": os.path.join(self.out_dir, "table_bayes_credible_interval_coverage.csv"), "table_bayes_credible_interval_coverage_tex": os.path.join(self.out_dir, "table_bayes_credible_interval_coverage.tex"), "table_bayes_diagnostics_csv": os.path.join(self.out_dir, "table_bayes_diagnostics.csv"), "table_bayes_diagnostics_tex": os.path.join(self.out_dir, "table_bayes_diagnostics.tex"), "plot_bayes_posterior_recovery_png": os.path.join(self.out_dir, "plot_bayes_posterior_recovery.png"), "plot_bayes_interval_summary_png": os.path.join(self.out_dir, "plot_bayes_interval_summary.png"), "bayes_report_summary_json": bayes_summary_path}
    ...
    def _bayes_posterior_rows(self, bayes_res):
        ...
        coverage = bayes_res.get("credible_interval_coverage", {})
        for param in ["theta", "psi0", "alpha"]:
            stats = summary.get(param, {})
            cov = coverage.get(param, {}) if isinstance(coverage, Mapping) else {}
            true_value = truth.get(param)
            ae = abs_error.get(param)
            rel_error = None
            if true_value is not None and ae is not None and abs(float(true_value)) > 1e-12:
                rel_error = float(ae) / abs(float(true_value))
            rows.append({"parameter": param, "true_value": true_value, "posterior_mean": stats.get("mean"), "posterior_median": stats.get("median"), "posterior_std": stats.get("std"), "p05": stats.get("p05"), "p95": stats.get("p95"), "covered_90pct": cov.get("covered_90pct"), "abs_error": ae, "relative_error": rel_error})
        return rows

    def _bayes_prior_to_posterior_rows(self, bayes_res):
        rows = []
        prior_summary = bayes_res.get("prior_summary", {})
        summary = bayes_res.get("posterior_summary", {})
        for param in ["theta", "psi0", "alpha"]:
            prior = prior_summary.get(param, {}) if isinstance(prior_summary, Mapping) else {}
            post = summary.get(param, {}) if isinstance(summary, Mapping) else {}
            prior_std = prior.get("std")
            post_std = post.get("std")
            sd_ratio = None
            if prior_std is not None and post_std is not None and abs(float(prior_std)) > 1e-12:
                sd_ratio = float(post_std) / float(prior_std)
            mean_shift = None
            if prior.get("mean") is not None and post.get("mean") is not None:
                mean_shift = float(post.get("mean")) - float(prior.get("mean"))
            rows.append({"parameter": param, "prior_family": prior.get("family"), "prior_mean": prior.get("mean"), "prior_std": prior.get("std"), "posterior_mean": post.get("mean"), "posterior_std": post.get("std"), "posterior_median": post.get("median"), "posterior_p05": post.get("p05"), "posterior_p95": post.get("p95"), "posterior_to_prior_sd_ratio": sd_ratio, "posterior_minus_prior_mean": mean_shift})
        return rows

    def _bayes_coverage_rows(self, bayes_res):
        rows = []
        coverage = bayes_res.get("credible_interval_coverage", {})
        for param in ["theta", "psi0", "alpha"]:
            cov = coverage.get(param, {}) if isinstance(coverage, Mapping) else {}
            rows.append({"parameter": param, "true_value": cov.get("true_value"), "ci_level": cov.get("ci_level"), "ci_lower": cov.get("ci_lower"), "ci_upper": cov.get("ci_upper"), "covered_90pct": cov.get("covered_90pct"), "posterior_mean": cov.get("posterior_mean"), "posterior_median": cov.get("posterior_median"), "absolute_error_mean": cov.get("absolute_error_mean"), "relative_error_mean": cov.get("relative_error_mean")})
        return rows
    ...
    def _plot_jacobian_singular_values(self, method_records, out_path):
        """Plot GMM/SMM moment-Jacobian singular values on a log scale."""
        labels = []
        series = []
        for rec in method_records:
            vals = np.asarray(dict(rec.get("jacobian_diagnostics", {})).get("singular_values", []), dtype=float)
            if vals.size:
        ...
        for vals, label in zip(series, labels):
            idx = np.arange(1, vals.size + 1)
            ax.plot(idx, np.maximum(vals, 1.0e-300), marker="o", label=label)
        ax.set_yscale("log")
        ax.set_xlabel("singular-value index")
        ax.set_ylabel("singular value (log scale)")
        ax.set_title("Moment-Jacobian singular values")
