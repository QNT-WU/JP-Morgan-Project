...
def _nelder_mead(
    ...
    """Small resumable Nelder-Mead optimizer with bound projection.

    The checkpoint stores the current simplex, function values, evaluation count,
    and convergence metadata after each completed simplex update.  This is not a
    SciPy-internal optimizer-state checkpoint; it is an exact checkpoint for this
    lightweight in-house Nelder-Mead routine, which is sufficient to resume SMM
    and GMM local starts after Colab interruptions.
    """
    ...
    def save_state(
        ...
        evals: int,
        converged: bool,
        simplex_diam: float = float("inf"),
        f_spread: float = float("inf"),
    ) -> None:
        """Persist the current local optimizer state for interruption recovery."""
        if checkpoint_path is None:
            return
        os.makedirs(os.path.dirname(checkpoint_path), exist_ok=True)
        payload = {
            "simplex": np.asarray(simplex, dtype=float).tolist(),
            "fvals": np.asarray(fvals, dtype=float).tolist(),
            "evals": int(evals),
            "converged": bool(converged),
            "simplex_diameter": float(simplex_diam),
            "f_spread": float(f_spread),
            "max_evals": int(max_evals),
            "tol": float(tol),
        }
        tmp_path = checkpoint_path + ".tmp"
        with open(tmp_path, "w", encoding="utf-8") as fh:
            json.dump(make_json_serializable(payload), fh, indent=2)
        os.replace(tmp_path, checkpoint_path)

    def load_state() -> Optional[Dict[str, object]]:
        """Load an incomplete local optimizer checkpoint when available."""
        if checkpoint_path is None or not os.path.exists(checkpoint_path):
            return None
        try:
            with open(checkpoint_path, "r", encoding="utf-8") as fh:
                payload = json.load(fh)
            simplex_arr = np.asarray(payload["simplex"], dtype=float)
            fvals_arr = np.asarray(payload["fvals"], dtype=float)
            if simplex_arr.ndim != 2 or fvals_arr.ndim != 1:
                return None
            if simplex_arr.shape[0] != fvals_arr.shape[0]:
                return None
            return {
                "simplex": simplex_arr,
                "fvals": fvals_arr,
                "evals": int(payload.get("evals", simplex_arr.shape[0])),
    ...
    if loaded is not None:
        simplex = np.asarray(loaded["simplex"], dtype=float)
        fvals = np.asarray(loaded["fvals"], dtype=float)
        evals = int(loaded["evals"])
        converged = bool(loaded.get("converged", False))
        print(
            f"[NelderMeadResume] loaded incomplete local optimizer state "
            f"from {checkpoint_path} at evals={evals}.",
            flush=True,
        )
    else:
        simplex = [x0]
        ...
        save_state(simplex, fvals, evals, converged, simplex_diam, f_spread)
    ...
    if checkpoint_path is not None and os.path.exists(checkpoint_path):
        # Keep the in-progress checkpoint only until the completed start JSON is
        # written by the caller.  The caller may remove it after successful save.
        pass
    return {
        "x": simplex[0],
...
def forward_simulate_dataset(
    ...
    for t in range(T):
        ...
        q = _price_q(qnet, z, k_next, b_next, mp, tp)
...
def _checkpoint_eval_count(checkpoint_path: Optional[str]) -> int:
    """Return the saved evaluation count for an incomplete local optimizer."""
    if checkpoint_path is None or not os.path.exists(checkpoint_path):
        return 0
    try:
        with open(checkpoint_path, "r", encoding="utf-8") as fh:
            payload = json.load(fh)
        return int(payload.get("evals", 0))
    except Exception:
        return 0
...
def _finalize_smm_variant(
    ...
    # Rows are SMM target moments and columns are estimated structural parameters.
    jacobian_diagnostics = jacobian_singular_value_report(
        g_num,
        param_names=param_names,
        moment_names=list(eval_hat.get("moment_names", [])),
    )
    a = g_num.T @ w @ g_num
    ...
    # Written-summary SMM specification statistic with simulation-error adjustment:
    #   J_T^SMM = T * J/(1+J) * g(Theta)' W g(Theta),
    # where J is the simulated/observed sample-size ratio.
    T_obs = max(1, n_obs_moments)
    J_adj = sim_ratio_J / (1.0 + sim_ratio_J) if sim_ratio_J > 0.0 else 1.0
    spec_stat = float(T_obs * J_adj * (g_gap @ w @ g_gap))
    spec_dof = int(max(0, len(m_obs) - len(param_names)))
    spec_p_value = float(_chi2_dist.sf(spec_stat, spec_dof)) if (_chi2_dist is not None and spec_dof > 0 and np.isfinite(spec_stat)) else float("nan")
    ...
    return {
        ...
        "specification_p_value": spec_p_value,
        "specification_reject_5pct": bool(spec_p_value < 0.05) if np.isfinite(spec_p_value) else False,
...
def estimate_smm(
    ...
    synth_path = os.path.join(out_dir, "smm_synth_data.npz")
    if os.path.exists(synth_path):
        if reporter is not None:
            reporter.start_phase("synthetic_data_generation", action="reuse_cached", path=synth_path)
        data = dict(np.load(synth_path))

        # The SMM objective evaluator needs the common-random-number shock panel
        # used for candidate simulations.  New cached datasets store this panel
        # under the ``eps`` key.  Older caches created before v25 do not contain
        # it, so we reconstruct the deterministic panel from the original seed.
        # This preserves resume compatibility without forcing users to delete
        # already-generated synthetic data.
        if "eps" in data:
            eps = np.asarray(data["eps"], dtype=np.float32)
        else:
        ...
        if reporter is not None:
            reporter.finish_phase(
                ...
                n_obs=int(np.asarray(data["k"]).reshape(-1).size),
                reused=True,
            )
        print(f"[SMM] reused cached synthetic dataset at {synth_path}", flush=True)
    else:
        rng = np.random.default_rng(seed)
        eps = rng.normal(0.0, mp_true.sigma_eps, size=(sim_n_paths, sim_T + 1)).astype(np.float32)
        ...
        np.savez_compressed(synth_path, **data, eps=eps)
        if reporter is not None:
    ...
    stage1_runs = _run_multistart(
        ...
        resume_dir=os.path.join(out_dir, "resume", "smm", "stage1_identity_weight"),
    )
    ...
    def make_stage2_runs(omega_data: np.ndarray, phase_name: str) -> List[Dict[str, object]]:
        ...
        return _run_multistart(
            ...
            progress_reporter=reporter,
            phase_name=phase_name,
            resume_dir=os.path.join(out_dir, "resume", "smm", phase_name),
        )
    ...
    res: Dict[str, object] = {
        ...
        "computational_controls": inner.computational_controls(),
