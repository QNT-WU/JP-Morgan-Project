"""Classical grid-based benchmark solvers for the risky-debt model."""
...
class GridBenchParams:
    """Discretization settings for the classical risky-debt benchmark."""
    Nk: int = 60
    Nb: int = 61
    Nz: int = 7
    # Nk: int = 15
    ...
    # The outer fixed-point loop for risky pricing often needs substantially
    # more iterations than the inner Bellman solve, especially after training a
    # neural policy that visits a wider state domain. A larger default budget
    # reduces spurious benchmark non-convergence in full experiment runs.
    outer_max_iter: int = 80
    tol_q: float = 1e-3
    damping: float = 0.95
...
def _rouwenhorst_markov(N: int, rho: float) -> np.ndarray:
    """Construct the Rouwenhorst Markov approximation for the shock process."""
    p = (1 + rho) / 2
    q = p
    P = np.array([[1.0]])
...
def build_z_grid_and_P(mp: ModelParams, Nz: int, z_m: float):
    """Build the Rouwenhorst productivity grid and transition matrix."""
    rho = mp.rho
    sigma = mp.sigma_eps
...
def _profit_pi(k: np.ndarray, z: np.ndarray, theta: float) -> np.ndarray:
    """Return operating profits for the current capital and productivity states."""
...
def _investment_I(k: np.ndarray, k_next: np.ndarray, delta: float) -> np.ndarray:
    """Return investment implied by the capital transition law."""
...
def _adj_cost_psi(I: np.ndarray, k: np.ndarray, psi0: float) -> np.ndarray:
    """Return the convex capital-adjustment cost."""
...
def _recovery_R(k_next: np.ndarray, z_next: np.ndarray, mp: ModelParams) -> np.ndarray:
    """Return lender recovery in default states."""
...
def _eta(e: np.ndarray, mp: ModelParams) -> np.ndarray:
    ...
    # eta(e) = -cost
    """Return the external-equity issuance adjustment applied to payouts."""
...
def _solve_inner(
    ...
    """Solve the inner firm problem for a fixed risky-pricing schedule."""
    V = V0.copy()
    ...
    if method == "vi":
        ...
        for _ in range(gp.inner_max_iter):
            ...
            inner_error_history.append(float(err))
            V = V_new
        ...
        return {
            ...
            "pol_k": pol_k,
            "pol_b": pol_b,
            "inner_error_history": np.asarray(inner_error_history, dtype=np.float64),
            "policy_change_history": np.asarray([], dtype=np.float64),
        }
    ...
    if method == "mpi":
        ...
        for _ in range(gp.mpi_max_iter):
            V_before_eval = V.copy()
            for _ in range(gp.mpi_eval_sweeps):
                EV = (V.reshape(-1, Nz) @ P.T).reshape(Nk, Nb, Nz)
                ...
                for iz in range(Nz):
                    ...
                    p_solv = np.tensordot(surv, P[iz, :], axes=([2], [0]))
                    debt_inflow = b[None, :] * q_z
                    term_profit = (1 - mp.tau) * pi[:, iz]
                    bpos = (b[None, :] > 0.0).astype(np.float64)
                    tax_shield_mn = beta * mp.tau * r_tilde * (b[None, :]) * p_solv * bpos
                    for i in range(Nk):
                        for j in range(Nb):
                            m = pol_k[i, j, iz]
                            n = pol_b[i, j, iz]
            ...
            changed_nodes = np.count_nonzero(pk_new != pol_k) + np.count_nonzero(pb_new != pol_b)
            policy_change_history.append(float(changed_nodes))
            inner_error_history.append(float(np.max(np.abs(V_new - V_before_eval))))
        ...
        return {
            ...
            "pol_b": pol_b,
            "inner_error_history": np.asarray(inner_error_history, dtype=np.float64),
            "policy_change_history": np.asarray(policy_change_history, dtype=np.float64),
        }
...
def _benchmark_latest_paths(checkpoint_dir: str) -> tuple[str, str]:
    """Return metadata and state paths for a benchmark resume directory."""
    return os.path.join(checkpoint_dir, "LATEST.json"), os.path.join(checkpoint_dir, "state_latest.npz")
...
def _save_benchmark_checkpoint(
    ...
    *,
    ...
    V: np.ndarray,
    C: np.ndarray | None,
    pol_k: np.ndarray | None,
    pol_b: np.ndarray | None,
    q_grid: np.ndarray,
    outer_q_error_history: list[float],
    inner_error_history: np.ndarray,
    policy_change_history: np.ndarray,
) -> None:
    """Save a resumable checkpoint for the benchmark outer pricing loop."""
    os.makedirs(checkpoint_dir, exist_ok=True)
    meta_path, state_path = _benchmark_latest_paths(checkpoint_dir)
    np.savez_compressed(
        state_path,
        V=V,
        C=np.asarray(C) if C is not None else np.asarray([], dtype=np.float64),
        pol_k=np.asarray(pol_k) if pol_k is not None else np.asarray([], dtype=np.int32),
        pol_b=np.asarray(pol_b) if pol_b is not None else np.asarray([], dtype=np.int32),
        q_grid=q_grid,
        outer_q_error_history=np.asarray(outer_q_error_history, dtype=np.float64),
        inner_error_history=np.asarray(inner_error_history, dtype=np.float64),
        policy_change_history=np.asarray(policy_change_history, dtype=np.float64),
    )
    with open(meta_path, "w", encoding="utf-8") as fh:
        json.dump(
            {
                "component": "grid_benchmark",
                "method": str(inner_method),
...
def _load_benchmark_checkpoint(checkpoint_dir: str, inner_method: str):
    """Load a benchmark checkpoint, returning ``None`` when unavailable."""
    meta_path, state_path = _benchmark_latest_paths(checkpoint_dir)
    if not (os.path.exists(meta_path) and os.path.exists(state_path)):
        return None
    try:
        with open(meta_path, "r", encoding="utf-8") as fh:
            meta = json.load(fh)
        if str(meta.get("method")) != str(inner_method):
            return None
        data = np.load(state_path, allow_pickle=False)
        return {
            "next_outer_iter": int(meta.get("next_outer_iter", 0)),
            "V": np.asarray(data["V"]),
            "C": None if np.asarray(data["C"]).size == 0 else np.asarray(data["C"]),
            "pol_k": None if np.asarray(data["pol_k"]).size == 0 else np.asarray(data["pol_k"]),
            "pol_b": None if np.asarray(data["pol_b"]).size == 0 else np.asarray(data["pol_b"]),
            "q_grid": np.asarray(data["q_grid"]),
            "outer_q_error_history": np.asarray(data["outer_q_error_history"], dtype=np.float64).tolist(),
            "inner_error_history": np.asarray(data["inner_error_history"], dtype=np.float64),
            "policy_change_history": np.asarray(data["policy_change_history"], dtype=np.float64),
        }
    except Exception:
        return None
...
def solve_grid_benchmark(
    ...
    # grids
    """Solve the risky-debt benchmark with the requested inner method."""
    ...
    # initialize or resume the outer risky-pricing fixed-point loop
    t0 = time.time()
    resume_state = _load_benchmark_checkpoint(checkpoint_dir, inner_method) if checkpoint_dir else None
    if resume_state is not None:
        V = resume_state["V"]
        C = resume_state["C"]
        pol_k = resume_state["pol_k"]
        pol_b = resume_state["pol_b"]
        q_grid = resume_state["q_grid"]
        outer_q_error_history = list(resume_state["outer_q_error_history"])
        inner_error_history = np.asarray(resume_state["inner_error_history"], dtype=np.float64)
        policy_change_history = np.asarray(resume_state["policy_change_history"], dtype=np.float64)
        start_outer_iter = int(resume_state["next_outer_iter"])
        if verbose:
            print(f"[GridBenchmark-{inner_method}] resumed at outer iteration {start_outer_iter:03d}", flush=True)
    else:
        V = np.zeros((gp.Nk, gp.Nb, gp.Nz), dtype=np.float64)
        q_grid = np.ones((gp.Nz, gp.Nk, gp.Nb), dtype=np.float64) * (1.0 / (1.0 + mp.r))
        C = None
        pol_k = pol_b = None
        outer_q_error_history = []
        inner_error_history = np.asarray([], dtype=np.float64)
        policy_change_history = np.asarray([], dtype=np.float64)
        start_outer_iter = 0

    for it in range(start_outer_iter, gp.outer_max_iter):
        inner = _solve_inner(inner_method, V, q_grid, pi, I, psi, k, b, P, mp, gp)
        V, C, pol_k, pol_b = inner["V"], inner["C"], inner["pol_k"], inner["pol_b"]
        inner_error_history = np.asarray(inner.get("inner_error_history", []), dtype=np.float64)
        policy_change_history = np.asarray(inner.get("policy_change_history", []), dtype=np.float64)

        q_comp = _update_q(C, R, P, b, mp)
        q_next = gp.damping * q_comp + (1.0 - gp.damping) * q_grid
        err = np.max(np.abs(q_next - q_grid))
        outer_q_error_history.append(float(err))
        q_grid = q_next
        ...
        if checkpoint_dir:
            _save_benchmark_checkpoint(
                checkpoint_dir,
                inner_method=inner_method,
                next_outer_iter=it + 1,
                V=V,
                C=C,
                pol_k=pol_k,
                pol_b=pol_b,
                q_grid=q_grid,
                outer_q_error_history=outer_q_error_history,
                inner_error_history=inner_error_history,
                policy_change_history=policy_change_history,
            )
        if verbose:
            print(f"[GridBenchmark-{inner_method}] outer {it:03d}  ||dq||inf={err:.3e}", flush=True)
        if err < gp.tol_q:
            break

    runtime_seconds = time.time() - t0
    default_star = (np.asarray(C) <= 0.0).astype(np.int32)

    return {
        "method": np.array(inner_method),
        "k_grid": k,
        "b_grid": b,
        "z_grid": z,
        "P": P,
        ...
        "pol_k_idx": pol_k,
        "pol_b_idx": pol_b,
        "policy_kp_star": k[pol_k],
        "policy_bp_star": b[pol_b],
        "q_star": q_grid,
        "outer_q_error_history": np.asarray(outer_q_error_history, dtype=np.float64),
        "inner_error_history": np.asarray(inner_error_history, dtype=np.float64),
        "policy_change_history": np.asarray(policy_change_history, dtype=np.float64),
        "outer_iterations": np.array(len(outer_q_error_history), dtype=np.int32),
