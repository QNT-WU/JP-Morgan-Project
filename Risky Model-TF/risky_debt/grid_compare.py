...
def nn_pricing_on_numpy(qnet_nn, z: np.ndarray, k_next: np.ndarray, b_next: np.ndarray) -> np.ndarray:
    """Evaluate the pricing network on NumPy inputs and return flattened prices."""
    import tensorflow as tf
...
def _pricing_grid_from_network(qnet, k_grid: np.ndarray, b_grid: np.ndarray, z_grid: np.ndarray) -> np.ndarray:
    """Evaluate the pricing network on the comparison grid."""
...
class BenchmarkComparator:
    ...
    def compare(
        ...
        *,
        ...
        q_nn_grid = _pricing_grid_from_network(qnet, k_grid, b_grid, z_grid)
