...
def plot_effectiveness_obj23(
    ...
    train_stat = _safe_get(hist, "train_stationarity_block")
    train_kkt = _safe_get(hist, "train_kkt_block")
    train_bprime = _safe_get(hist, "train_bprime_block")
    ...
    if train_stat is not None:
        _plot_series(
            e,
            train_stat,
            title=f"{obj_name}: TrainStationarityBlock",
            ylabel="TrainStationarityBlock",
            out_path=os.path.join(fig_dir, f"effectiveness_{stem_lower}_train_stationarity_block.png"),
            out_path_legacy=out_prefix + "_train_stationarity_block.png",
        )
    ...
    if train_kkt is not None:
        _plot_series(
            e,
            train_kkt,
            title=f"{obj_name}: TrainKKTBlock",
            ylabel="TrainKKTBlock",
            out_path=os.path.join(fig_dir, f"effectiveness_{stem_lower}_train_kkt_block.png"),
            out_path_legacy=out_prefix + "_train_kkt_block.png",
        )

    if train_bprime is not None:
        _plot_series(
            e,
            train_bprime,
            title=f"{obj_name}: TrainBPrimeBlock",
            ylabel="TrainBPrimeBlock",
            out_path=os.path.join(fig_dir, f"effectiveness_{stem_lower}_train_bprime_block.png"),
            out_path_legacy=out_prefix + "_train_bprime_block.png",
        )
