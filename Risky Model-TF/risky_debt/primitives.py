...
@tf.function
def issuance_eta_exact(e: tf.Tensor, eta0: float, eta1: float) -> tf.Tensor:
    """Exact external-equity issuance cost contribution.

    The exact economic rule is used in final nonsmoothed evaluation:
    eta_exact(e) = -[eta0 + eta1 (-e)] 1{e < 0}.
    """
    e = tf.convert_to_tensor(e, tf.float32)
    cost = (tf.cast(eta0, tf.float32) - tf.cast(eta1, tf.float32) * e)
    return -cost * tf.cast(e < 0.0, tf.float32)
...
def equity_cashflow_total_e(
    ...
    """Return current cash flow plus the discounted contingent tax shield.

    This helper is retained for backward compatibility.  It must not be used as
    the external-equity issuance-cost base.  The aligned model applies issuance
    costs to ``e^0`` only and adds the future tax shield separately in Bellman or
    rollout continuation terms.
    """
...
def equity_payout_d_total(
    ...
    """Return aligned period value: ``d^0 + PV(tax shield)``.

    Issuance costs are based on the current cash-flow object ``e^0`` only.  The
    future solvency-contingent tax shield is added after ``d^0`` is computed, so
    it does not reduce today's external-financing need.
    """
    e_base = equity_cashflow_base_e(k, k_next, b, b_next, z, q, mp)
    eta = issuance_eta(e_base, mp.eta0, mp.eta1, kappa_issue)
    tax_pv = interest_tax_shield_pv(
        b_next=b_next,
        q=q,
        continuation_weight=continuation_weight,
    ...
    return e_base + eta + tax_pv


@tf.function
def equity_payout_d_exact(
    k: tf.Tensor,
    k_next: tf.Tensor,
    b: tf.Tensor,
    b_next: tf.Tensor,
    ...
    """Exact current payout ``d_exact^0=e^0+eta_exact(e^0)``."""
    e_base = equity_cashflow_base_e(k, k_next, b, b_next, z, q, mp)
    return e_base + issuance_eta_exact(e_base, mp.eta0, mp.eta1)
