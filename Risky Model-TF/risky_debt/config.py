...
# @dataclass: automatically creates an __init__ like ModelParams(rho=..., sigma_eps=..., ...) and also nice printing.
# frozen=True: makes it immutable after creation. So you can’t accidentally do mp.r = 0.2 in the middle of training.
...
class ModelParams:
    ...
    # Code uses a strict interior [𝑞min⁡,𝑞max⁡] to avoid:
...
# his says: each network (policy/value/pricing) will typically be an MLP with
# 3 hidden layers
# 128 units each
# tanh activation
@dataclass(frozen=True)
...
# These match your “multi-term residual” definitions.
@dataclass(frozen=True)
class Obj1Params:
