...
def rollout_discounted_reward(
    ...
    # uses your model discounting: β=1/(1+r)​
    beta = tf.constant(beta_from_r(mp.r), tf.float32)
    # Ensures 𝑘>0 and 𝑧>0 numerically
    # b can be any real number (consistent with your model)
    ...
    for _ in tf.range(T + 1):
        ...
        # But in Objective 1 you do not have a trained 𝑉~ and 𝑉 (by design),
...
def _euler_residuals_autodiff(
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        # If no value: use just 𝑑. This is not the model’s Euler equation — it’s a stability proxy for Obj1 where you didn’t train a value network.
        if value is None:
