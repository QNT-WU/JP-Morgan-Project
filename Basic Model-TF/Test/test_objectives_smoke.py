...
from basic_mailer.config import ModelParams, NetParams, TrainParams, Obj2Params, Obj3Params
...
def test_obj2_loss_finite_and_gradients():
    ...
    npol = NetParams(hidden_units=8, hidden_layers=1, activation="tanh")
    pol = PolicyNet(npol, mp.k_min, mp.k_max)
    mul = MultiplierNet(npol)

    _ = pol(tf.zeros((1, 2), dtype=tf.float32))
    _ = mul(tf.zeros((1, 2), dtype=tf.float32))
    ...
    with tf.GradientTape() as tape:
        loss = obj2_batch_loss(pol, mul, mp, Obj2Params(), k, z)
    vars_ = pol.trainable_variables + mul.trainable_variables
...
def test_obj3_loss_finite_and_gradients():
    ...
    op3 = Obj3Params(nu_fb=1.0, nu_lambda=1.0)
    ...
    assert any(g is not None for g in grads_m)
