...
# generate the ergodic distribution sample of states (𝑘𝑡,𝑧𝑡) under the current policy network.
...
def simulate_ergodic_dataset(
    ...
    # These are Python lists that will accumulate TF tensors.
    ks, zs = [], []
    ...
    for t in range(total_steps + 1):
        # Before burn-in: do not store.
        # After burn-in: append k and z
        # Each appended k is shape [n]
        # So after the loop:
        # length of ks = tp.ergodic_T + 1 (because inclusive)
        # each element is [n]
        ...
        # clamp by k_min
        # That is: kt+1(i)​=max(φ(kt(i)​,zt(i)​),kmin​)
    ...
    # So you return:k_flat: shape [n*T_keep];z_flat: shape [n*T_keep]
    # This is your “ergodic dataset”: a big bag of states
    k_flat = tf.reshape(k_mat, (-1,)).numpy()
    z_flat = tf.reshape(z_mat, (-1,)).numpy()

    # cap the buffer size (memory control)
    # If you generated too many points, this:
