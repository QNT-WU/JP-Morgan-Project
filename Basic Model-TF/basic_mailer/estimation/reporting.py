...
def make_identification_rows(results: Mapping[str, object]) -> List[Dict[str, object]]:
    """Create local-identification diagnostics based on moment Jacobians."""
    rows: List[Dict[str, object]] = []
    for method in METHOD_ORDER:
        if method not in results:
            continue
        ...
        rows.append({
            'method': method,
            'moment_jacobian_rank': flat.get('moment_jacobian_rank', ''),
            'moment_jacobian_min_singular_value': flat.get('moment_jacobian_min_singular_value', ''),
            'moment_jacobian_condition_number': flat.get('moment_jacobian_condition_number', ''),
            'moment_jacobian_singular_values': flat.get('moment_jacobian_singular_values', ''),
            'final_success': bool(res.stage2.success),
        })
    return rows
...
def save_estimation_report(results: Mapping[str, object], true_params: Mapping[str, float], out_dir: str) -> None:
    ...
    identification_rows = make_identification_rows(results)
    ...
    write_csv(identification_rows, os.path.join(out_dir, 'table_identification_diagnostics.csv'))
    ...
    write_latex_table(identification_rows, os.path.join(out_dir, 'table_identification_diagnostics.tex'), caption='Local identification diagnostics based on moment Jacobians', label='tab:identification')
