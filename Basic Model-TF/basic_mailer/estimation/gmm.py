...
class TwoStepGMMEstimator:
    ...
    def _moment_series_from_tilde(self, theta_tilde: np.ndarray) -> np.ndarray:
        ...
        return compute_gmm_moment_series(
            self.observed_dataset,
            params["beta"],
            params["theta"],
            params["psi0"],
            self.mp_template.delta,
        )
    ...
    def _build_starts(
        self, x_center: np.ndarray, n_starts: int, start_scale: float
    ) -> List[np.ndarray]:
        ...
        for _ in range(max(0, n_starts - 1)):
            starts.append(
                x_center + self.rng.normal(scale=start_scale, size=x_center.shape)
            )
        return starts

    def _run_multistart(
        self, stage: str, starts: List[np.ndarray], W: np.ndarray, max_evals: int
    ) -> StageSummary:
        ...
        for sid, x0 in enumerate(starts):
            ...
            rec = StartRecord(
                stage,
                sid,
                list(np.asarray(x0, dtype=np.float64)),
                list(np.asarray(res.x, dtype=np.float64)),
                bool(res.success),
                int(res.status),
                str(res.message),
                int(res.nfev),
        ...
        return StageSummary(
            ...
            records,
            best_idx,
            transform_tilde_to_params(best_x),
            float(best_loss),
            bool(best_res["success"]),
            int(best_res["status"]),
    ...
    def fit(
        ...
        *,
        ...
        max_evals: int = 60,
        weight_methods: Iterable[str] = ("standard", "newey_west"),
        n_starts: int = 1,
        start_scale: float = 0.15,
    ) -> Dict[str, GMMMethodResult]:
        """Run the full estimation workflow and return method results."""
        if x0 is None:
            x0 = transform_params_to_tilde(
                **structural_params_from_model(self.mp_template)
            )
        W1 = np.eye(4, dtype=np.float64)
        starts1 = self._build_starts(
            np.asarray(x0, dtype=np.float64), n_starts=n_starts, start_scale=start_scale
        )
        stage1 = self._run_multistart("stage1", starts1, W1, max_evals=max_evals)
        series_stage1 = self._moment_series_from_tilde(
            transform_params_to_tilde(**stage1.best_x)
        )
        results: Dict[str, GMMMethodResult] = {}
        total_t0 = time.perf_counter()
        for idx, method in enumerate(weight_methods):
            cov_info = estimate_weighting_matrix(
                series_stage1, method=method, ridge=self.ridge, lags=self.hac_lags
            )
            starts2 = self._build_starts(
                transform_params_to_tilde(**stage1.best_x),
                n_starts=n_starts,
                start_scale=start_scale,
            )
            stage2 = self._run_multistart(
                "stage2", starts2, cov_info.weight_matrix, max_evals=max_evals
            )
            ...
            V_tilde = sandwich_parameter_covariance(
                D,
                cov_info.covariance,
                cov_info.weight_matrix,
                self.n_moment_obs,
                simulation_adjustment=1.0,
                ridge=self.ridge,
            )
            ...
            delta = np.diag(
                [
                    params_vec[0] * (1.0 - params_vec[0]),
                    params_vec[1] * (1.0 - params_vec[1]),
                    1.0 / (1.0 + np.exp(-t_vec[2])),
                ]
            )
            ...
            results[method_name] = GMMMethodResult(
                method_name,
                cov_info.method,
                stage1,
                stage2,
                stage2.best_x,
                {str(i): float(v) for i, v in enumerate(g_final)},
                cov_info,
                V_struct,
