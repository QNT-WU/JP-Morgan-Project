...
def _train_policy_obj2_inner(*, mp: ModelParams, npol: NetParams, tp: TrainParams, warm_start_policy: Optional[PolicyNet] = None) -> PolicyNet:
    """Train an Objective 2 policy model used inside SMM candidate evaluation.

    The helper mirrors the standalone Objective 2 training loop but keeps the
    implementation local to SMM so candidate parameter vectors can be solved
    and simulated during estimation.
    """
...
class TwoStepSMMEstimator:
    ...
    def _simulate_candidate(self, theta_tilde: np.ndarray) -> tuple[PathDataset, ModelParams]:
        """Solve and simulate the model under one candidate parameter vector."""
        mp_candidate = self._candidate_model_params(theta_tilde)
        policy = _train_policy_obj2_inner(mp=mp_candidate, npol=self.npol, tp=self.inner_tp, warm_start_policy=None)
        ds_candidate = simulate_paths_crn(policy=policy, mp=mp_candidate, design=self.simulation_design, burn_in=0)
        return ds_candidate, mp_candidate

    def _moment_vector_from_tilde(self, theta_tilde: np.ndarray) -> np.ndarray:
        """Compute simulated SMM target moments for an unconstrained parameter vector."""
        ds_candidate, mp_candidate = self._simulate_candidate(theta_tilde)
        m_dict = compute_moments(ds_candidate, mp_candidate, self.spec)
        return np.asarray([m_dict[name] for name in self.spec.names], dtype=np.float64)
    ...
    def fit(self, *, x0: np.ndarray | None = None, max_evals: int = 40, weight_methods: Iterable[str] = ("standard", "newey_west"), n_starts: int = 1, start_scale: float = 0.15) -> Dict[str, SMMMethodResult]:
        ...
        for idx, method in enumerate(weight_methods):
            ...
            results[method_name] = SMMMethodResult(method_name, cov_info.method, stage1, stage2, stage2.best_x, self.observed_moments, sim_moments, {name: float(gap) for name, gap in zip(self.spec.names, gaps)}, self.observed_summary, summarize_smm_moments(sim_moments), cov_info, V_struct, params_from_vector(se), float(self.simulation_adjustment), float(time.perf_counter() - total_t0))
        return results
