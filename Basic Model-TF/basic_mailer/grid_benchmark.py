"""Grid-based benchmark construction for the basic dynamic program."""
...
def make_k_grid(Nk: int, k_min: float, k_max: float) -> np.ndarray:
    """Construct the capital grid used by the benchmark solver."""
    return np.linspace(k_min, k_max, Nk)


# ----------------------------
# Step 0.2 + 0.3: shared plumbing
# ----------------------------
@dataclass(frozen=True)
class GridSpec:
    """Describe the benchmark grid discretization."""
    Nk: int = 200
    Nz: int = 7
    k_min: float = 0.05
    k_max: float = 8.0
...
@dataclass
class Precomputed:
    """Store precomputed benchmark arrays shared by solvers."""
...
def solve_vfi(
    ...
    for _it in range(max_iter):
        ...
        Q = pre.u + pre.beta * EV[None, :, :].transpose(
            0, 2, 1
        )  # careful? Let's do explicitly:
        ...
        # easiest: loop over m but vectorized over i,j (fast enough for Nk<=300,Nz<=9)
    ...
    info = {
        "method": "vfi",
        "converged": converged,
        "iterations": len(diff_history),
        "tol": float(tol),
        "final_diff": float(diff_history[-1]) if diff_history else float("nan"),
        "diff_history": diff_history,
    }
    if return_info:
        return V, policy_idx, info
    return V, policy_idx
...
def solve_howard_pi(
    ...
    for _outer in range(max_outer):
        ...
        changes = int(np.sum(new_policy != policy_idx))
        value_diff = float(np.max(np.abs(policy_evaluation_sweeps(pre, new_policy, V_init=V, n_sweeps=1) - V)))
        changes_history.append(changes)
        value_diff_history.append(value_diff)
    ...
    info = {
        "method": "howard_pi",
        "converged": converged,
        "outer_iterations": len(changes_history),
        "tol_policy": int(tol_policy),
        "eval_sweeps": int(eval_sweeps),
        "final_policy_changes": int(changes_history[-1]) if changes_history else 0,
        "policy_changes_history": changes_history,
        "value_diff_history": value_diff_history,
    }
    if return_info:
        return V, policy_idx, info
    return V, policy_idx
...
def policy_from_idx(k_grid: np.ndarray, policy_idx: np.ndarray) -> np.ndarray:
    """Convert benchmark action indices into next-capital levels."""
