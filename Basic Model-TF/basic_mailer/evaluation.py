"""Evaluation helpers for rewards and Euler-residual diagnostics."""

from __future__ import annotations

# Same purpose as before: postpones evaluation of type hints.
...
def rollout_discounted_reward(
    ...
    return W
    # returns discounted reward for each of the N paths: shape [N].
...
def euler_f_policy_only(
    ...
    """Evaluate the policy-only Euler residual kernel."""
    beta = tf.constant(beta_from_r(mp.r), tf.float32)
    ...
    # Note: This uses mp.sigma_eps in the draw step, not here.
    z_next = tf.exp(mp.rho * tf.math.log(z) + eps)

    # k1 = k' = φ(k,z)
    # k2 = k'' = φ(k', z')
    # This is required because the RHS includes terms involving next period investment I1
    x = tf.stack([k, z], axis=1)
    # k1 = tf.maximum(policy(x), mp.k_min)
    k1 = tf.clip_by_value(policy(x), mp.k_min, mp.k_max)

    x1 = tf.stack([k1, z_next], axis=1)
    # k2 = tf.maximum(policy(x1), mp.k_min)
    k2 = tf.clip_by_value(policy(x1), mp.k_min, mp.k_max)
...
def euler_residual_with_value_derivative(
    ...
    """Evaluate the Euler residual using a value-function derivative."""
    beta = tf.constant(beta_from_r(mp.r), tf.float32)
    k = tf.maximum(k, mp.k_min)
    z = tf.maximum(z, 1e-12)
    ...
    # Why v_sum? Because value(xw) returns a vector (one value per sample).
...
# It is similar to Obj2 test but does not square the conditional mean;
# it squares the residual itself across state-shock pairs.
