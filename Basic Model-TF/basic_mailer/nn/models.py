...
@tf.keras.utils.register_keras_serializable(package="basic_mailer")
class MultiplierModel(tf.keras.Model):
    """Nonnegative KKT multiplier network for the capital lower bound.

    The model maps the state ``(k_t, z_t)`` to a nonnegative multiplier
    ``lambda(k_t, z_t)``. A softplus readout enforces dual feasibility by
    construction, which is required by the KKT/Fischer--Burmeister objectives.
    """
    ...
    def __init__(self, net_params: NetParams, **kwargs):
        ...
        Args:
            ...
            **kwargs: Extra keyword arguments forwarded to ``Model``.
        """
        kwargs.setdefault("name", "multiplier_model")
        super().__init__(**kwargs)
        self.net_params = net_params
        self.state_normalizer = StateNormalization()
        self.backbone = MLPBlock(
        ...
        self.readout = tf.keras.layers.Dense(1, activation=None, name="multiplier_readout")

    @tf.function(reduce_retracing=True)
    def call(self, inputs: tf.Tensor, training: bool = False) -> tf.Tensor:
        """Return nonnegative multiplier values for a batch of states.
        ...
        Returns:
            A rank-1 tensor containing one nonnegative multiplier per state.
        """
        x = self.state_normalizer(inputs)
        h = self.backbone(x, training=training)
        raw = self.readout(h, training=training)
        lam = tf.nn.softplus(raw)
